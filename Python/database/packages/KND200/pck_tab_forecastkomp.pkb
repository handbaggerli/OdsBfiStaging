create or replace package body pck_tab_forecastkomp is
  ---------------------------------  Header  ----------------------------
  -- Name         :  pck_tab_forecastkomp
  -- Projekt      :  BFI Release 3
  -- Beschreibung :  Aufbereitung fuer ODS_BFI
  --              :  Aufbereitung fuer Tabelle forecastkomp
  -- Template:    :  KND200 - pck_tab_forecastkomp.pkb --> Entspricht wieder dem Template
  -- Autor        :  MMe
  -- Erstelldatum :  24.05.2017
  -- Auftrags-Nr. :  ??-?????
  --
  -- Skript-Name  :  $Id: pck_tab_forecastkomp.pkb 8984 2017-06-08 13:55:43Z U10938 $
  -- Revision
  -- Nr.          :  $Revision: 8984 $
  -- User         :  $Author: U10938 $
  -- Datum        :  $Date: 2017-06-08 15:55:43 +0200 (Do, 08 Jun 2017) $
  -- HeadURL      :  $HeadURL: http://10.10.10.66/svn/DWH_Repository/trunk/DWH_ALLGEMEIN/SYRIUS_R3_DWH/OdsBfiStaging/database/packages/KND200/pck_tab_forecastkomp.pkb $
  --
  -- Aenderungsnachweis :
  ...
  begin
    ...
    if p_steuerung.table_ladetyp_next = pck_def_enviorement_variables.c_load_type_full then
      execute immediate 'truncate table forecastkomp';

      pck_warte_constraint_info.prc_drop_all ('forecastkomp');
    else
      delete forecastkomp
      where pkey in (select pkey
                     from {staging_schema}.forecastkomp as of scn (p_verarbeitung.aktueller_scn)
  ...
  begin
    ...
    if l_return then
      ...
      if p_steuerung.table_ladetyp_next = pck_def_enviorement_variables.c_load_type_full then
        pck_warte_constraint_info.prc_create_all ('forecastkomp');
        pck_ods_statistcs.prc_gather_table_stats ('forecastkomp');
      end if;
    ...
    if p_steuerung.pruefen = pck_def_enviorement_variables.c_pruefen_yes then
      ...
      where 1 = 1
              and pkey not in (select pkey_exclude
                         from t_exclude_list
                         where upper(table_name) = 'FORECASTKOMP');
