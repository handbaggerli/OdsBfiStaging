create or replace package body pck_tab_code is
  ---------------------------------  Header  ----------------------------
  -- Name         :  pck_tab_code
  -- Projekt      :  BFI Release 3
  -- Beschreibung :  Aufbereitung fuer ODS_BFI
  --              :  Aufbereitung fuer Tabelle CODE
  -- Template     :  Generell - pck_tab_code.pkb
  -- Autor        :  MMe
  -- Erstelldatum :  23.05.2017
  -- Auftrags-Nr. :  ??-?????
  --
  -- Skript-Name  :  $Id: pck_tab_code.pkb 8984 2017-06-08 13:55:43Z U10938 $
  -- Revision
  -- Nr.          :  $Revision: 8984 $
  -- User         :  $Author: U10938 $
  -- Datum        :  $Date: 2017-06-08 15:55:43 +0200 (Do, 08 Jun 2017) $
  -- HeadURL      :  $HeadURL: http://10.10.10.66/svn/DWH_Repository/trunk/DWH_ALLGEMEIN/SYRIUS_R3_DWH/OdsBfiStaging/database/packages/pck_tab_code.pkb $
  --
  -- Aenderungsnachweis :
  ...
  begin
    ...
    if p_steuerung.table_ladetyp_next = pck_def_enviorement_variables.c_load_type_full then
      ...
      pck_warte_constraint_info.prc_drop_all ('code');
    else
      delete code
      where pkey in (select pkey
                     from {staging_schema}.code as of scn (p_verarbeitung.aktueller_scn)
  ...
  begin
    ...
    insert /*+ append */
      ...
      from {staging_schema}.code as of scn (p_verarbeitung.aktueller_scn)
      where     1 = 1
            and replaced = to_date('01.01.3000','dd.mm.yyyy') 
    ...
    if l_return then
      ...
      if p_steuerung.table_ladetyp_next = pck_def_enviorement_variables.c_load_type_full then
        pck_warte_constraint_info.prc_create_all ('code');
        pck_ods_statistcs.prc_gather_table_stats ('code');
      end if;
